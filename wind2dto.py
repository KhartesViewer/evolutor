...
class ImageViewer(QLabel):
    ...
    def wxysToIxys(self, wxys):
        ...
        cx,cy = self.center
        z = self.zoom
        ixys = np.zeros(wxys.shape, dtype=np.float32)
        ixys[...,0] = cx + dxys[...,0]/z
        ixys[...,1] = cy + dxys[...,1]/z
        return ixys
    ...
    def loadTIFF(self, fname):
        try:
            image = cv2.imread(str(fname), cv2.IMREAD_UNCHANGED).astype(np.float32)
            image /= 65535.
        except Exception as e:
            print("Error while loading",fname,e)
            return
        self.image = torch.from_numpy(image).to(self.gpu)
    ...
    def drawAll(self):
        ...
        if st is not None and self.dip_bars_visible and scale == 1:
            ...
            uvs = st.interpolator(st.vector_u, gdpir, mode='nearest').cpu().numpy()
            vvs = st.interpolator(st.vector_v, gdpir, mode='nearest').cpu().numpy()
            coherence = st.interpolator(st.linearity, gdpir, mode='nearest').cpu().numpy()
