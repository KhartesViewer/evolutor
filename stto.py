...
class ST(object):
    ...
    def __init__(self, image):
        self.image = image
        print("ST image", image.shape, image.dtype, image.device)
        self.lambda_u = None
        self.lambda_v = None
        self.vector_u = None
        self.vector_v = None
    ...
    # creates kernel of 2*radius+1, where radius = sigma*num_sigmas
    @staticmethod
    def gaussian_kernel_1d(sigma, num_sigmas = 3., device=None):
    ...
    def gaussian_filter_2d(img, sigma):
        ...
        return img.squeeze_(0).squeeze_(0)  # Make 2D again
        '''
        return ST.separable_filter_2d(img, kernel_1d, kernel_1d)
    ...
    def showArray(arr):
        ...
        # bim = PIL.Image.fromarray((arr.cpu().numpy()*255).astype(np.uint8))
        bim = PIL.Image.fromarray((oarr.cpu().numpy()).astype(np.uint8))
        bim.show()

    @staticmethod
    def computeEigensTorch(gx2, gxy, gy2):
    ...
    @staticmethod
    def computeEigensExplicit(gx2, gxy, gy2):
    ...
    def computeTensors(self):
        ...
        hksize = ksize//2
        cv2_kernel = cv2.getGaussianKernel(ksize, sigma0)
        print("cv2 kernel")
        print(cv2_kernel)
        to_kernel = self.gaussian_kernel_1d(sigma0, device=self.image.device)
        print("to_kernel")
        print(to_kernel)
        '''
        # kernel = self.gaussian_kernel_1d(sigma0, device=self.image.device)
        gkernel = self.gaussian_kernel_1d(sigma0, device=img.device)
        ngkernel = gkernel.cpu().numpy()
        dkernel = ngkernel.copy()
        ksize = ngkernel.shape[0]
        hksize = ksize // 2
        ...
        gx2 = self.gaussian_filter_2d((gx*gx), sigma1)
        gxy = self.gaussian_filter_2d((gx*gy), sigma1)
        gy2 = self.gaussian_filter_2d((gy*gy), sigma1)

        # print("gx2", gx2.shape, gx2.min(), gx2.max())
        # self.showArray(gx2)
        # self.showArray(gy2)
        # self.showArray(gxy)
